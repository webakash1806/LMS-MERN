...
const CourseCard = ({ data }) => {
    ...
    return (
        <div className='w-[17.5rem] h-[26rem] cursor-pointer flex mt-2 flex-col border items-start gap-2 rounded-md overflow-hidden'
            onClick={() => navigate("/LMS-Client/course/description")}>
            <img className='w-full h-[44%] object-cover' src={data?.thumbnail?.secure_url} alt={`${data?.title} image`} />
            <div className='text-white flex flex-col gap-2 p-3'>
                <h2 className='text-[1.2rem] font-[600]'>{data?.title}</h2>
